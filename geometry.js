...
function zoomAt(canvas, px, py, deltaSteps) {
  ...
  const sensitivity = 0.08 * (1 + Math.log10(1 + kmPerPx) * 0.8);
...
export function attachPanZoom(canvas) {
  ...
  canvas.addEventListener(
    ...
    (e) => {
      e.preventDefault();
      const steps = -Math.sign(e.deltaY) * Math.min(0.6, Math.abs(e.deltaY) / 220);
      zoomAt(canvas, canvas.clientWidth / 2, canvas.clientHeight / 2, steps);
    },
    { passive: false }
  );
